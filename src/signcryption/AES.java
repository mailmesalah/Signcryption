...
public class AES {
	...
	private void createAESKey() {
		...
		if (K2.length < AES_KEY_LENGTH)
			copylength = K2.length;
	...
	/**
	 * Encrypts current unix_timestamp, padding and the message in this order
	 * with 4 bytes + 1 byte + message_byte.length
	 * 
	 * @param message
	...
	protected byte[] encryptTimePaddingMessage(String message) {
		try {
			...
			byte[] res = new byte[unix_timestamp_length + padding_length
					+ byte_message.length];

			long unix_timestamp = System.currentTimeMillis() / 1000L;
			byte[] unix_timestamp_bytes = BigInteger.valueOf(unix_timestamp)
					.toByteArray();
			int position = 0;
			System.arraycopy(unix_timestamp_bytes, 0, res, position,
					unix_timestamp_length);
			position += unix_timestamp_length;

			int padding = 16 - ((unix_timestamp_length + padding_length + byte_message.length) % 16);
	...
	/**
	 * Function especially made for the class Cryptogram, that encrypts and sets
	 * the cleartext, msglength and unix timestamp field of the current
	 * Cryptogram.
	 * 
	 * @param cipher
	 * @param current
	...
	protected byte[] decryptTimestampAndMessage(byte[] cipher,
		...
		byte[] res = new byte[cipher.length - unix_timestamp_length
				- padding_length];
		byte[] unix_timestamp_bytes = new byte[unix_timestamp_length];

		int position = 0;
		System.arraycopy(cleartext, position, unix_timestamp_bytes, 0,
				unix_timestamp_length);
		position += unix_timestamp_length;
		...
		current.setUnixTimestamp(new BigInteger(unix_timestamp_bytes)
				.longValue());
