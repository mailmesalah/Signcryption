...
public class SigncryptPacket {
	...
	public SigncryptPacket(Cryptogram c, ECPoint r, BigInteger s,
	...
	private byte[] makeBytes() {
		...
		packetAsBytes = new byte[settings.getAppPreambleByteSize()
				+ settings.getVersionByteSize() + 1 + (int) RBytesLength + 1
				+ (int) sBytesLength + cBytes.length];
		int position = 0;
		...
		System.arraycopy(RBytes, 0, packetAsBytes, position, (int) RBytesLength);
		position += (int) RBytesLength;

		packetAsBytes[position] = sBytesLength;
		position += 1;

		System.arraycopy(sBytes, 0, packetAsBytes, position, (int) sBytesLength);
		position += (int) sBytesLength;

		System.arraycopy(cBytes, 0, packetAsBytes, position, cBytes.length);
	...
	private void fetchFromBytes() {
		byte[] appPreambleBytes = new byte[settings.getAppPreambleByteSize()];
		byte[] versionBytes = new byte[settings.getVersionByteSize()];

		int position = 0;
		System.arraycopy(packetAsBytes, position, appPreambleBytes, 0,
				settings.getAppPreambleByteSize());
		position += settings.getAppPreambleByteSize();

		System.arraycopy(packetAsBytes, position, versionBytes, 0,
		...
		System.arraycopy(packetAsBytes, position, RBytes, 0, RBytesLength);
		...
		System.arraycopy(packetAsBytes, position, sBytes, 0, sBytesLength);
		position += sBytesLength;

		byte[] cBytes = new byte[packetAsBytes.length - position];
	...
	public String toString() {
		return "R(" + R.getX().toBigInteger() + "," + R.getY().toBigInteger()
				+ "), \ns(" + s + "(" + s.bitLength() + ")) , "
				+ "\nencrypted message(" + new String(c.getEncrypted()) + ")";
	}
}