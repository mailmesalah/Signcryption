...
public class SigncryptionManager extends AbstractSigncrypt {
	...
	public SigncryptionManager(SigncryptionSettings settings) throws Exception {
		super(settings);
	}
	...
	public void generateKeyPair() {
		...
		privateKeyAscii85 = Ascii85Coder.encodeBytesToAscii85(privateKey
				.toByteArray());
		System.out.println(privateKeyAscii85);
	}

	public String getPublicKeyAsAscii85() {
		return Ascii85Coder.encodeBytesToAscii85(new ECPoint.Fp(publicKey
				.getCurve(), publicKey.getX(), publicKey.getY(), true)
				.getEncoded());
	}
	 ...
	 * @return
	 */
	public BigInteger setPrivateKeyFromEncryptedAscii85(String ascii85,
			String password) {
		byte[] bytes = Ascii85Coder.decodeAscii85StringToBytes(ascii85);
		AES aes = new AES(password.getBytes(), settings.kl);
	...
	public void setKeyPairFromEncryptedPrivateKey(String ascii85key,
			String password) {
		AES aes = new AES(password.getBytes(), settings.kl);
		privateKey = new BigInteger(aes.encrypt(
				Ascii85Coder.decodeAscii85StringToBytes(ascii85key), false));
		publicKey = G.multiply(privateKey);
	}
}